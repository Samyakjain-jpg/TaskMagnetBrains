      ...
      <div className="row">
        ...
        {tasks.length > 2 && (
          <div className="pagination">
            <span onClick={() => selectPageHandler(page - 1)} className={page > 1 ? "" : "disabled"}>◀</span>
            {[...Array(Math.ceil(tasks.length / 8))].map((_, i) => (
              <span key={i} onClick={() => selectPageHandler(i + 1)} className={page === i + 1 ? "selected" : ""}>
                {i + 1}
              </span>
            ))}
            <span onClick={() => selectPageHandler(page + 1)} className={page < Math.ceil(tasks.length / 8) ? "" : "disabled"}>▶</span>
          </div>
        )}
      </div>
